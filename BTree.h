//
// Created by 姜永铭 on 1/7/16.
//

#ifndef XINNUOTEST_BTREE_H
#define XINNUOTEST_BTREE_H
...
protected:
    ...
    BTNodePosi(T) _root;
    BTNodePosi(T) _hot;
    void solvOverflow(BTNodePosi(T));
    void solvUnderflow(BTNodePosi(T));
...
template <typename T>
void BTree<T>::solvOverflow(BTNodePosi(T) v) {
...
template <typename T>
void BTree<T>::solvUnderflow(BTNodePosi(T) v ) {
    ...
    if(0<r){
        ...
        while (!v->key.empty()) {
            ls->key.insert(ls->child.size(),v->key.remove(0));
            ls->child.insert(ls->child.size() ,v->child.remove(0) );
